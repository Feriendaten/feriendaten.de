defmodule FeriendatenWeb.FaqTest do
  ...
  test "GET /", %{conn: conn} do
    ...
    assert html_response(conn, 200) =~ "Warum Herbstferien kürzer?"
    assert html_response(conn, 200) =~ "Warum nur 1 Woche Herbstferien Hessen?"
    ...
    assert html_response(conn, 200) =~ "FAQ"
    refute html_response(conn, 200) =~ "Warum Herbstferien kürzer?"
    refute html_response(conn, 200) =~ "Warum nur 1 Woche Herbstferien Hessen?"
  end
end
